...
/**
 * 兵种基类.
 * 所有兵种（BaseTroop）的抽象基类，不可直接实例化，只能继承.
 */
...
public:
    ...
    /** @return 当前攻击目标. */
    BaseBuilding* getAttackTarget() const { return _attackTarget; }

    /** @param target 设置攻击目标. */
    void setAttackTarget(BaseBuilding* target);

    /** @param targetTilePos 设置目标瓦片坐标. */
    void setTargetTilePosition(const cocos2d::Vec2& targetTilePos);

    /** @param targetPos 设置目标世界坐标并切换至移动状态. */
    ...
    /** 兵种特有攻击行为（如炸弹人炸墙、弓箭手远程攻击）. */
    virtual void doSpecialAttack() = 0;

    /** * 获取特有描述.
     * @return 描述字符串（如“优先攻击城墙，造成范围伤害”）.
     */
