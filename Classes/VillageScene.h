            ...
            if (lines.size() >= 4 && !lines[3].empty()) {
                ...
                if (resParts.size() >= 6) {
                    try {
                        ...
                        data.gold = 0; data.elixir = 0; data.poplation = 0;
                        data.maxGold = 0; data.maxElixir = 0; data.maxPopulation = 0;
                    }
...
public:
    ...
    /** * 读档（从本地文件恢复）.
     * @param savePath 存档路径.
     * @param fallbackToResources 是否回退到资源文件.
     * @return 成功返回 true.
     */
    ...
    /** 增加容量. */
    void addTroopCapacity(int bonus);
    ...
    /** 显示训练弹窗. */
    void showTrainingCampPopup(TrainingCamp* camp);

    /** 隐藏训练弹窗. */
    void hideTrainingCampPopup();
...
private:
    ...
    /** 初始化一切按钮. */
    void initBtns(BaseMode baseMode = BaseMode::CREATING);
